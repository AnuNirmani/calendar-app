...
if (isset($_GET['delete'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $check_posts_stmt = $conn->prepare("SELECT COUNT(*) as post_count FROM posts WHERE category_id = ?");
        $check_posts_stmt->bind_param("i", $category_id);
        $check_posts_stmt->execute();
        $posts_result = $check_posts_stmt->get_result();
        $posts_data = $posts_result->fetch_assoc();
        $post_count = $posts_data['post_count'];
        $check_posts_stmt->close();
        
        if ($post_count > 0) {
            $_SESSION['error'] = "Cannot delete category '$category_name' because it has $post_count post(s) associated with it. Please reassign or delete the posts first.";
        } else {
...
// Get category statistics and post counts
...
foreach ($categories as $category) {
    ...
    $count_stmt = $conn->prepare("SELECT COUNT(*) as post_count FROM posts WHERE category_id = ?");
    $count_stmt->bind_param("i", $category_id);
    $count_stmt->execute();
    $count_result = $count_stmt->get_result();
    $count_data = $count_result->fetch_assoc();
    $category_post_counts[$category_id] = $count_data['post_count'];
    $count_stmt->close();
}
?>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    <?php foreach ($categories as $category): 
                                        $post_count = $category_post_counts[$category['id']] ?? 0;
                                    ...
                                    ?>
                                        <tr class="table-row border-b">
                                            ...
                                            <td class="p-4 border border-gray-300">
                                                <?php if ($post_count > 0): ?>
                                                    <span class="post-count-badge" title="<?php echo $post_count; ?> post(s) in this category">
                                                        <i class="fas fa-file-alt mr-1"></i>
                                                        <?php echo $post_count; ?> post<?php echo $post_count == 1 ? '' : 's'; ?>
                                                    </span>
                                                <?php else: ?>
                                                    <span class="text-gray-400 text-sm">No posts</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="p-4 border border-gray-300">
                                            ...
                                            <td class="p-4 border border-gray-300">
                                                <div class="flex justify-center space-x-2">
                                                    ...
                                                    <?php if ($has_posts): ?>
                                                        <span class="px-4 py-2 bg-red-300 text-white rounded flex items-center btn-disabled"
                                                              title="Cannot delete - category has <?php echo $post_count; ?> post(s)">
