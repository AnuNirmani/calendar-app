...
if (isset($_FILES['upload']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
        $url = '/uploads/ckeditor/' . $fileName; // Adjust URL based on your server
        echo json_encode(['uploaded' => 1, 'fileName' => $fileName, 'url' => $url]);
        exit;
    } else {
        echo json_encode(['uploaded' => 0, 'error' => ['message' => 'Error uploading file']]);
        exit;
    }
}
...
<head>
    ...
    <!-- CKEditor CDN with Image Upload -->
    ...
    <script>
        $(document).ready(function() {
            ...
            ClassicEditor
                .create(document.querySelector('#postContent'), {
                    extraPlugins: [ClassicEditor.uploadPlugin],
                    image: {
                        toolbar: ['imageTextAlternative', 'imageStyle:full', 'imageStyle:side', 'linkImage'],
                        upload: {
                            url: 'add_post.php' // Point to this file for image upload handling
                        }
                    }
                })
                .then(editor => {
            ...
            $("#postForm").validate({
                ...
                submitHandler: function(form) {
                    ...
                    if (window.editor) {
                        ...
                        $('<textarea>').attr({ name: 'postContent', type: 'hidden' }).val(content).appendTo(form);
                    }
                    form.submit();
                }
            });
        ...
        function saveDraft() {
            if ($("#postForm").valid()) {
                ...
                if (window.editor) {
                    const content = editor.getData();
                    $('<textarea>').attr({ name: 'postContent', type: 'hidden' }).val(content).appendTo('#postForm');
                }
