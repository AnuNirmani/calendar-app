...
if (!empty($search)) {
    ...
    $where_condition = " WHERE c.name LIKE '%$search_clean%' 
                         OR c.status LIKE '%$search_clean%'";
...
$sql .= " GROUP BY c.id ORDER BY c.created_at DESC LIMIT $offset, $records_per_page";
            ...
            <div class="max-w-6xl mx-auto">
                ...
                <div class="flex justify-between mb-3">
                    <form method="GET" action="" class="flex w-full max-w-md">
                        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by category name or status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <button type="submit" class="ml-2 py-2 px-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Search</button>
                        <?php if (!empty($search)): ?>
                            <a href="list_categories.php" class="ml-2 py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-700">Clear</a>
                        <?php endif; ?>
                    </form>
                </div>
                    ...
                    <?php if (empty($categories)): ?>
                        ...
                        <?php if (!empty($search)): ?>
                            <div class="text-center">
                                <a href="list_categories.php" class="text-blue-600 hover:text-blue-800 underline">View all categories</a>
                            </div>
                        <?php endif; ?>
                    <?php else: ?>
