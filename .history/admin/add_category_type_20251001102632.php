...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Custom validation method to check for duplicate categories
            $.validator.addMethod("uniqueCategory", function(value, element) {
                var isDuplicate = false;
                var currentId = $('#categoryId').val();
                
                $('#categoriesTable tbody tr').each(function() {
                    var rowId = $(this).attr('id');
                    if (rowId) {
                        var categoryId = rowId.replace('category-', '');
                        var categoryName = $(this).find('td:eq(1)').text().trim().toLowerCase();
                        
                        // Skip the current category if editing
                        if (currentId && categoryId === currentId) {
                            return true;
                        }
                        
                        if (categoryName === value.trim().toLowerCase()) {
                            isDuplicate = true;
                            return false;
                        }
                    }
            ...
            $('#categoryName').on('keyup blur', function() {
                $(this).valid();
            });
        });
