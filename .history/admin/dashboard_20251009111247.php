    ...
    <div class="flex h-screen">
        ...
        <div class="main-content flex-1 p-4 md:p-8 md:ml-[250px] gradient-bg">
            ...
            <div class="max-w-full md:max-w-5xl mx-auto">
                ...
                <p class="text-lg text-gray-600 mb-8">Manage your categories and posts efficiently. Use the sidebar to navigate or start by adding a new category below.</p>
                ...
                <div class="bg-white p-8 rounded-xl shadow-lg mb-6">
                    ...
                    <form method="POST" action="">
                        <input type="text" name="category_name" class="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600" placeholder="Enter category name" required>
                        <button type="submit" class="mt-4 bg-indigo-600 text-white py-2 px-4 rounded-lg hover:bg-indigo-700">Create Category</button>
                    </form>
                ...
                <div class="bg-white p-8 rounded-xl shadow-lg">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-6">Recent Categories</h2>
                    <?php
                    $categories = getCategories();
                    if (isset($categories['error'])) {
                        echo "<p class='text-red-600'>{$categories['error']}</p>";
                    } elseif (empty($categories)) {
                        echo "<p class='text-gray-500'>No categories found.</p>";
                    } else {
                        echo "<ul class='list-disc pl-5 text-gray-600'>";
                        foreach ($categories as $category) {
                            echo "<li class='mb-2'>{$category['name']} (<span class='capitalize'>{$category['status']}</span>)</li>";
                        }
                        echo "</ul>";
                    }
                    ?>
