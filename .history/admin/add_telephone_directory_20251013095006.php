...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($phone_number) || empty($department_id)) {
        ...
        if ($stmt_check->get_result()->num_rows == 0) {
            ...
            if ($stmt->execute()) {
                $success = "Telephone entry added successfully!";
            } else {
...
$department_list = $departments && $departments->num_rows > 0 ? $departments : false;
if (!$department_list) {
    ...
    <div class="flex h-screen">
        ...
        <div class="main-content flex-1 p-8">
            <div class="max-w-4xl mx-auto">
                <h2 class="text-3xl font-bold text-gray-800 mb-6">Add Telephone Directory</h2>
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <form method="POST" action="">
                            <div class="mb-3">
                                ...
                                <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500" id="name" name="name" required>
                            </div>
                            <div class="mb-3">
                                <label for="phone_number" class="block text-sm font-medium text-gray-700">Phone Number</label>
                                <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500" id="phone_number" name="phone_number" required pattern="[0-9]{10}">
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500" id="department_id" name="department_id" required>
                                    ...
                                    <?php while ($row = $department_list->fetch_assoc()): ?>
                                        <option value="<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['name']); ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <button type="submit" class="py-2 px-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Add Telephone Directory</button>
                        </form>
                    </div>
                <?php endif; ?>
            </div>
